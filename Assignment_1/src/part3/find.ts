...
export const findResult: <T>(pred: (x: T) => boolean, arr: T[]) => Result<T> = <T>(pred: (x: T) => boolean, arr: T[]) => 
    arr.length === 0 ? makeFailure("Failure.") :

    pred(arr[0]) ? makeOk(arr[0]) :
...
export const returnSquaredIfFoundEven_v2 = (arr: number[]): Result<number> => 
    bind(findResult((x) => x % 2 === 0, arr), (x: number) => makeOk(x * x)); 


export const returnSquaredIfFoundEven_v3 = (arr: number[]): number => 
    either(findResult((x) => x % 2 === 0, arr), (x: number) => x * x, (s: string) => -1);
